...
double intersectCPC(double r, double z, 
		    double vr, double vphi, double vz) 
//double CPCgeometry::intersect(PathState* pathstate)
{
	const double a = 0.04; // m
	const double g = 9.80665; // m/s^2
	const double m = 10.454077; // neV s^2/m^2
	const double mg = m*g;
	//double vphi = pathstate->getAngularVelocity();

	//double gm = g + (vr^2 + vphi^2)/(2a);
	double gm = g + (vr*vr + vphi*vphi)/(2*a);
	...
	double P = -1/12*alpha*alpha - gamma;
	double Q = -1/108*alpha*alpha*alpha + 1/3*alpha*gamma - 1/8*beta*beta;
	...
	double chiplus = sqrt(-zeta+xi);
	double chiminus = sqrt(-zeta-xi);
	double roots[] = {
		tvz + (omega + chiminus)/2, 
		tvz + (omega - chiminus)/2, 
		tvz - (omega - chiplus)/2, 
		tvz - (omega + chiplus)/2
	};
